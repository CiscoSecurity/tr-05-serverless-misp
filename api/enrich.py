...
def get_judgement(observable_value,observable_type,disposition,valid_time,disp,attribute):
...
def get_sightings(observable_value,observable_type,disposition,attribute):
    ...
    return{
        ...
        'relations':get_relations(observable_value,observable_type,disposition),
        'schema_version':'1.1.3',
...
def get_indicators(observable_value,observable_type,disposition,attribute,valid_time):
...
def get_relations(observable_value,observable_type,disposition):
    ...
    observable_values={'url':'url','ip-src':'ip','sha256':'sha256','domain':'domain'}
    d={'origin':'MISP Intelligence',
    'relation':'related-to',
    ...
    for i in attribute:
        ...
        if(i['type'] in observable_values and i['value']!=observable_value):
            x['related']={'value':i['value'], 'type':observable_values[i['type']]}
            relations.append(x)
    return relations
...
def observe_observables():
    ...
    for observable in observables:
        ...
        g.judgements.append(get_judgement(o_value, o_type, disposition_tuple, valid_time,disposition,attribute))
        sight=get_sightings(o_value, o_type,disposition,attribute)
        g.sightings.append(sight)
        indicate=get_indicators(o_value,o_type,disposition,attribute,valid_time)
        g.indicators.append(indicate)
        g.relationships.append(get_relationships(sight['id'],indicate['id'],disposition))
        if g.verdicts:
