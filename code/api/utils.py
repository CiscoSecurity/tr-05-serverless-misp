...
JWKS_HOST_MISSING = "jwks_host is missing in JWT payload. Make sure " "custom_jwks_host field is present in module_type"
WRONG_JWKS_HOST = (
    "Wrong jwks_host in JWT payload. Make sure domain follows " "the visibility.<region>.cisco.com structure"
)
...
def filter_observables(observables):
    ...
    return list(
        filter(
            lambda obs: (obs["type"] in supported_types and obs["value"] != "0"),
            observables,
        )
    )
