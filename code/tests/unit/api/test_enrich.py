...
def test_enrich_call_with_valid_jwt_but_invalid_json_value(
    ...
    response = client.post(route, headers=get_headers(valid_jwt()), json=invalid_json_value)
    assert response.status_code == HTTPStatus.OK
    assert response.json == invalid_json_expected_payload("{0: {'value': ['Field may not be blank.']}}")
...
def test_enrich_call_with_unsupported_type_json(
    ...
    mock_request.return_value = EXPECTED_RESPONSE_OF_JWKS_ENDPOINT
    response = client.post(route, headers=get_headers(valid_jwt()), json=unsupported_type_json)
    assert response.status_code == HTTPStatus.OK
